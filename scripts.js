...
connection.on("receive", (id, value) =>
  {
    ...
    if (id === "a07675fa-dd41-4846-a0cd-08dd18680dfa") {
        updateTemperatureIndicator(parseFloat(value));
    }

    if (id === "7966f39c-ceae-4946-a0cc-08dd18680dfa") {
        updateFillingIndicator(parseFloat(value));
    }

    if (id === "cdb6e556-8803-4365-a0ce-08dd18680dfa") {
        updateHumidityIndicator(parseFloat(value));
    }

    if (id === "a66515d1-a21e-4aac-a0d5-08dd18680dfa") {
        updatepHIndicator(parseFloat(value));
    }
...
function updateTemperatureIndicator(value) {
    const light = document.getElementById('temperature-light');
...
function updateFillingIndicator(value) {
    const light = document.getElementById('filling-light');
    const valueElement = document.getElementById('filling-value');
    valueElement.textContent = `${value}%`;

    if (value > THRESHOLDS.filling) {
        light.style.backgroundColor = 'red';
    } else {
        light.style.backgroundColor = 'green';
    }
...
function updateHumidityIndicator(value) {
    const light = document.getElementById('humidity-light');
...
function updatepHIndicator(value) {
    const light = document.getElementById('pH-light');
    const valueElement = document.getElementById('pH-value');
    valueElement.textContent = `${value}%`;

    if (value > THRESHOLDS.pH) {
        light.style.backgroundColor = 'red';
    } else {
        light.style.backgroundColor = 'green';
    }
